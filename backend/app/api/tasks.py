...
def create_detection_task(
    ...
    background_tasks.add_task(task_service.run_detection, task.id)
    return task


@router.get("", response_model=list[DetectTask])
def list_tasks(store: JsonStore = Depends(get_store)) -> list[DetectTask]:
    return store.list_tasks()


@router.get("/{task_id}", response_model=DetectTask)
def get_task(task_id: str, store: JsonStore = Depends(get_store)) -> DetectTask:
